...
{
  var confirmBox = confirm('Are you sure to remove selected examiner?' + "\n" + 'Once deleted, it can\'t be restored!');
  if (confirmBox) {
...
if (isset($_GET['removesess'])) {
  $idx = $_GET['removesess'];
  unset($_SESSION['biblioExaminer'][$idx]);
  echo '<script type="text/javascript">';
  echo 'alert(\''.__('Examiner succesfully removed!').'\');';
  echo 'location.href = \'iframe_examiner.php\';';
  echo '</script>';
}
...
if (isset($_POST['remove'])) {
  ...
  $sql_op->delete('biblio_examiner', 'examiner_id='.$id.' AND biblio_id='.$bid);
  echo '<script type="text/javascript">';
  echo 'alert(\''.__('Examiner removed!').'\');';
  echo 'location.href = \'iframe_examiner.php?biblioID='.$bid.'\';';
  echo '</script>';
}
...
if ($biblioID) {
  ...
  // database list
  $biblio_examiner_q = $dbs->query("SELECT be.*, a.examiner_name,a.examiner_number, a.examiner_type FROM biblio_examiner AS be
      LEFT JOIN mst_examiner AS a ON be.examiner_id=a.examiner_id
      WHERE be.biblio_id=$biblioID ORDER BY level ASC");
  $row = 1;
  while ($biblio_examiner_d = $biblio_examiner_q->fetch_assoc()) {
    // alternate the row color
    $row_class = ($row%2 == 0)?'alterCell':'alterCell2';

    // remove link
    $remove_link = '<a href="#" class="notAJAX btn button btn-danger btn-delete" onclick="confirmProcess('.$biblioID.', '.$biblio_examiner_d['examiner_id'].')">Delete</a>';
    $examiner            = $biblio_examiner_d['examiner_name'];
    $examiner_number      = $biblio_examiner_d['examiner_number'];
    $examiner_type        = $sysconf['examiner_type'][$biblio_examiner_d['examiner_type']];

    $table->appendTableRow(array($remove_link, $examiner, $examiner_number, $examiner_type, $sysconf['authority_level_examiner'][$biblio_examiner_d['level']]));
  ...
  if ($_SESSION['biblioExaminer']) {
    ...
    foreach ($_SESSION['biblioExaminer'] as $biblio_session) {
      // remove link
      $remove_link = '<a class="notAJAX btn button btn-danger btn-delete" href="iframe_examiner.php?removesess='.$biblio_session[0].'">Remove</a>';

      if ($biblio_session) {
          $examiner_q = $dbs->query("SELECT examiner_name, examiner_number, examiner_type FROM mst_examiner
              WHERE examiner_id=".$biblio_session[0]);
          $examiner_d             = $examiner_q->fetch_row();
          $examiner               = $examiner_d[0];
          $examiner_number        = $examiner_d[1];
          $examiner_type          = $examiner_d[2];
      }

      $table->appendTableRow(array($remove_link, $examiner, $examiner_number, $examiner_type, $sysconf['authority_level_examiner'][$biblio_session[1]]));
