...
$gmd_q = $dbs->query('SELECT gmd_name FROM mst_gmd LIMIT 50');
while ($gmd_d = $gmd_q->fetch_row()) {
    echo '<li><a href=index.php?title=&search=search&author=&subject=&isbn=&gmd='.$gmd_d[0].'&searchtype=advance&search=search>'.$gmd_d[0].'</a></li>';
}
$gmd_href = ob_get_clean();

/* tipe koleksi List href */
ob_start();
$tipe_q = $dbs->query('SELECT item_type_name FROM mst_item_type LIMIT 50');
while ($tipe_d = $tipe_q->fetch_row()) {
    echo '<li><a href=index.php?title=&search=search&author=&subject=&isbn=&itemtype='.$tipe_d[0].'&searchtype=advance&search=search>'.$tipe_d[0].'</a></li>';
}
$tipe_koleksi = ob_get_clean();
